...
const videoschema = {
	_id:       { required: false }, // id of video
	userid:    { required: true  }, // id of user who posted video
	likes:     { required: false  }, // number of likes for this video
	desc:      { required: false }, // description of video
	...
	timestamp: { required: false  }  // timestamp of video
};
const commentschema = {
	_id:       { required:false }, // id of comment
	...
	likes:     { required:false  }, // number of likes for this comment
...
function validate(obj, schema) {
   return obj && Object.keys(schema).every( 
		field =>
			!schema[field].required || obj[field]
